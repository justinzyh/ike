...
public class VerticalRollingTextView extends TextView{
    ...
    private String TAG = "VerticalRollingBean";
    private int  mDuration; //文字从出现到显示消失的时间
    private int  mInterval; //文字停留在中间的时长切换的间隔
    private List<VerticalRollingBean> mTexts; //显示文字的数据源
    ...
    private OnClickTener onClickLitener;

    public void setOnClickLitener(OnClickTener onClickLitener) {
    ...
    private void init() {
        ...
        mPaintFront.setTextSize(16);

        mPaintBack = new Paint();
        mPaintBack.setAntiAlias(true);
        mPaintBack.setDither(true);
        mPaintBack.setTextSize(16);
    }
    ...
    protected void onDraw(Canvas canvas) {
        if (mTexts != null) {
            Log.i(TAG, "onDraw: " + mY);
            VerticalRollingBean model = (VerticalRollingBean) mTexts.get(mIndex);
